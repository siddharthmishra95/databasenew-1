...
$sel = mysqli_query($conn,"select count(*) as allcount from crud WHERE  data LIKE '%2022' AND status='OK' ");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of records with filtering
$sel = mysqli_query($conn,"select count(*) as allcount from crud WHERE  data LIKE '%2022' AND  status='OK'  ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "select * from crud WHERE  data LIKE '%2022' AND status='OK'  ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
...
while ($row = mysqli_fetch_assoc($empRecords)) {
    $data[] = array(
            ...
            "edit"=>'<a href="../candidate/addautocandidate.php?edit='.$row['id'].'" height="60" width="75" class="btn btn-warning bt-xs update" />Edit</a>',
            "shortlist"=>'<a href="shortlist.php?shortlist='.$row['id'].'" height="60" width="75" class="btn btn-info bt-xs update" />Shortlist</a>',
    		"duplicate"=>'<a href="../candidate/addautocandidate.php?duplicate='.$row['id'].'" height="60" width="75" class="btn btn-danger bt-xs update" />Duplicate</a>',
    	);
}
