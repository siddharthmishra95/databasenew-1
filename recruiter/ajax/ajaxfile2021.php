...
$sel = mysqli_query($conn,"select count(*) as allcount from crud WHERE  data LIKE '%2021' AND status='OK' ");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of records with filtering
$sel = mysqli_query($conn,"select count(*) as allcount from crud WHERE  data LIKE '%2021' AND  status='OK'  ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "select * from crud WHERE  data LIKE '%2021' AND status='OK'  ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
